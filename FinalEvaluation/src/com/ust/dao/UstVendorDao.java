...
public class UstVendorDao {
	...
	public VendorDetails getVendorById(int vendor_id) {
		...
		// return template
		// .query("select vc.vendor_id,vc.vendor_name,vc.address,vc.location,vc.service,vc.pincode,cd.name,cd.department,cd.email,cd.phone,cd.contact_id from vendor vc join contactdetails cd on vc.vendor_id=cd.vendor_id where vc.isActive=0 and vc.vendor_id='"+
		// vendor_id + "'",
		// new RowMapper<VendorDetails>() {
		// public VendorDetails mapRow(ResultSet rs, int row)
		// throws SQLException {
